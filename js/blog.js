...
function setupDynamicBlogNavigation() {
	...
	async function fetchAndDisplayContent(url, isPushState = true, isProject = false) {
		...
		// REDIRECT FIX: Block unwanted URLs from being loaded
		if (url && (url.includes('YouTube-Timecode-Commentary') || url.includes('/2025/06/25/'))) {
			console.log('[Blog] Blocked fetchAndDisplayContent for unwanted URL:', url);
			return;
		}
		...
		try {
			...
			if (newContentContainer) {
				...
				if (newContentContainer.classList.contains('project-edge-wrapper')) {
					...
					// Add class to help with styling projects
					if (isProject || newContentContainer.classList.contains('project-wrapper')) {
						innerWrapper.classList.add('with-project');
					}
				...
				// Re-initialize link listeners for dynamic content navigation
				initializeLinkListeners(blogContentElement);
			...
			if (isPushState) {
				...
				// Store current URL and tab state in sessionStorage for back navigation
				sessionStorage.setItem('previousUrl', window.location.href);
				if (isProject) {
					// Always store 'portfolio' as the previous tab when navigating FROM a project list
	...
	const handlePopstate = async function (event) {
		// REDIRECT FIX: Enhanced prevention of unwanted redirects
		if (event.state && event.state.path) {
			if (event.state.path.includes('YouTube-Timecode-Commentary') || 
			    event.state.path.includes('/2025/06/25/') ||
			    (event.state.isDynamic && window.location.pathname === '/')) {
				console.log('[Blog] Blocked unwanted redirect to:', event.state.path);
				// Reset to clean homepage state
				history.replaceState({ path: '/', isInitial: true, isDynamic: false }, '', '/');
				return;
			}
		}
		...
		if (!event.state || event.state.isInitial) {
			...
			if (blogContentElement) {
				...
				// Re-initialize blog features including tabs
				initializeBlogFeatures(blogContentElement);
			}
			
			// Check the previous URL to determine if we're coming back from a project
			const previousUrl = document.referrer || sessionStorage.getItem('previousUrl') || '';
			const wasOnProject = previousUrl.includes('/20'); // Year-based URLs
			
			// Also check if there's a stored previous tab state
			...
			if (wasOnProject) {
				...
				// Switch to portfolio tab since we're coming back from a project
...
function handleSearch(e) {
	// Searching posts
	// Use target if available from event, otherwise use passed element
	...
	let noResultsMessage = postsContainer.querySelector('.no-results-message');
	if (!noResultsMessage) {
		noResultsMessage = document.createElement('div');
		noResultsMessage.className = 'no-results-message';
		noResultsMessage.style.textAlign = 'center';
		noResultsMessage.style.padding = '3rem 2rem';
		noResultsMessage.style.color = 'var(--text-color-secondary)';
	...
	posts.forEach(post => {
		const postType = post.querySelector('.post-byline')?.textContent.includes('Musing') ? 'musing' : 'post';
...
// DOMContentLoaded handler for initial setup
document.addEventListener('DOMContentLoaded', function () {
	...
	console.log('[Blog] Running initial setup');
	...
	if (document.querySelector('.blog-content')) {
		...
		console.log('[Blog] No .blog-content found, skipping dynamic navigation setup');
	}
});

// Also run critical initialization immediately if DOM is already loaded
if (document.readyState !== 'loading') {
	console.log('[Blog] DOM already loaded, running immediate init');
	// Set up dynamic navigation immediately if blog-content exists
	if (document.querySelector('.blog-content')) {
		setupDynamicBlogNavigation();
	}
